...
FROM "Tenant" t
WHERE t."slug" = 'default'
    AND "Study"."tenantId" IS NULL;
UPDATE "StudyAttachment" sa
SET "tenantId" = s."tenantId"
FROM "Study" s
WHERE sa."studyId" = s."id"
...
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'Study_tenantId_fkey'
) THEN
ALTER TABLE "Study"
ADD CONSTRAINT "Study_tenantId_fkey" FOREIGN KEY ("tenantId") REFERENCES "Tenant"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
END IF;
END $$;
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'StudyAttachment_tenantId_fkey'
) THEN
ALTER TABLE "StudyAttachment"
ADD CONSTRAINT "StudyAttachment_tenantId_fkey" FOREIGN KEY ("tenantId") REFERENCES "Tenant"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
END IF;
END $$;
